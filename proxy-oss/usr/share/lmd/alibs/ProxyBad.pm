...
my $file = "/var/lib/squidGuard/db/custom/bad/domains";
...
{
	return [
		 { title        => 'Enhance the List of bad Domains' },
		 { type         => 'command' },
		 { allowedRole  => 'root' },
...
sub Save
{
	...
	if($reply->{warning}){
		...
		my @content = split /\n/, $text;
			...
			$this->execute("/usr/sbin/squidGuard -c /etc/squid/squidguard.conf -C custom/bad/domains");
