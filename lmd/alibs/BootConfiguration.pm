...
sub edit
{
	...
	my $msg_text = '';
		...
		my $hash = $this->get_ws_or_room_type($dn_ws,$ws_hwconf_tmp,@terminalservers);
                if( !exists($hash->{ws_or_room_type}) ){
			if($msg_text eq ''){
				$msg_text = main::__("The following computers don't have hardware configs set : ")."$hostname, ";
			}else{
	                        $msg_text .= "$hostname, ";
			}
                        push @lines, { line => [ $dn_ws,
                                       { workstation => $hostname },
                                       { wstype => $hash->{ws_or_room_type} },
                        ...
                        push @lines, { line => [ $dn_ws,
                                       { workstation => $hostname },
                                       { wstype => $hash->{ws_or_room_type} },
                                       { bootconfiguration => $hash->{bootconfig} },
                             ]};
                }
	}
	...
	my @ret;
	if($room_HW ){
		my $hash = $this->get_ws_or_room_type($reply->{line},$room_HW,@terminalservers);
		if(!exists($hash->{ws_or_room_type})){
			$msg_text .= "<BR>".main::__("There isn't a hardware configuration set for the classroom!");
...
#print "======== $hwconf_ws_or_room : $WSType :  $descrp : $bootconf_tmp \n";
		...
		push @{$return{bootconfig}}, @terminalservers, 'Local', 'cloneTool', 'Autoinstallation', '---DEFAULTS---', $bootconf_tmp->[0];
		$return{ws_or_room_type} = 'FatClient/'.$descrp;
        }elsif( $WSType eq 'ThinClient' ){
		push @{$return{bootconfig}}, @terminalservers , '---DEFAULTS---', $bootconf_tmp->[0];
		$return{ws_or_room_type} = 'ThinClient/'.$descrp;
        }elsif( $WSType eq 'LinuxTerminalServer' ){
		push @{$return{bootconfig}}, 'Local', 'Autoinstallation', '---DEFAULTS---', $bootconf_tmp->[0];
		$return{ws_or_room_type} = 'LinuxTerminalServer/'.$descrp;
        }elsif( $WSType eq 'WindowsTerminalServer' ){
		push @{$return{bootconfig}}, 'Local' , '---DEFAULTS---', $bootconf_tmp->[0];
		$return{ws_or_room_type} = 'WindowsTerminalServer/'.$descrp;
	}
