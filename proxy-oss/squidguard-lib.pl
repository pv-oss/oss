  ...
  for (my $i=0; $i < @c; $i++) {
    ...
    if ($c[$i] =~ /^dbhome\s+(\S+)/) {
      ...
      while(($i < scalar(@c)) && ($c[$i] !~ /^\s*\}/)) {
        ...
        } elsif ($c[$i] =~ /^execuserlist/) {
	  my( $a,$b) = split /\s+/,$c[$i],2;
          my %st;
...
my $Ocfg  = parse_config('/etc/squid/squidguard.conf.rpmsawe');
...
foreach my $owner ( keys %$ORIG )
{
	...
	push @{$ORIG->{$owner}->{newpass}}, 'all'  if( defined $ORIG->{$owner}->{pass}->{all} );
	push @{$ORIG->{$owner}->{newpass}}, 'none' if( defined $ORIG->{$owner}->{pass}->{none} );
}
