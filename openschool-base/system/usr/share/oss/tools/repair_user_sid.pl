...
sub usage
{
	print   'Usage: /usr/share/oss/tools/repair_user_sid.pl [OPTION]'."\n".
		'Leiras ....'."\n\n".
		'Options :'."\n".
...
$mesg = $oss->{LDAP}->search( base   => $oss->{LDAP_BASE},
		       ...
		       scope  => 'sub',
		       attrs  => ['sambaSID','sambaPrimaryGroupSID']
		     );
...
foreach my $entry ( $mesg->entries ) {
  ...
  my $PSID = $entry->get_value('sambaPrimaryGroupSID');
  if( !$PSID ) {
    print $entry->dn()."\n";
  } elsif( $PSID =~ s/$badSID/$goodSID/ ) {
     $oss->{LDAP}->modify( $entry->dn(), replace => { sambaPrimaryGroupSID => $PSID });
     print $entry->dn()." corrected\n";
  }
}
