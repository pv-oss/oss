...
sub changePassword
{
	...
	return [
		{ subtitle     => "Setting New Password for:" },
		{ label        => $list },
		{ userpassword => "" },
		{ mustchange   => 1 },
		{ sso          => 0 },
...
sub editUser
{
	...
	foreach my $attr ( @userAttributeList )
	{
		...
		if( defined $user->{$attr} )
		{
			...
			{ #we don't have to print role, description do contains it 
				next;
			}
...
#		my @i = ( grep { !/do_not_change/} @disabledChange,    $u->{internetdisabled}->[0] );
#		my @o = ( grep { !/do_not_change/} @enabledChange, $u->{oxenabled} );
#		push @list, { line => [ $dn ,	{ cn               => $u->{cn}->[0] }, 
#						{ mailenabled      => \@m },
#						{ logindisabled    => \@l },
#						{ internetdisabled => \@i },
#						{ oxenabled        => \@o }]};
